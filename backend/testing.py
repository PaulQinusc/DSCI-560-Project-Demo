def predict_func(user_destination, walking_distance):
    ...
    test_features = {'Normal_day': 1, "Day_type_Weekend": 1, "Month_4": 1, "Hour_20": 1}
    ...
    for i in avaiable_locations:
        i += ' Los Angeles, CA, USA'  # tom make sure there are in LA
    ...
    if origin == 'crypto arena':
        dest_dict = {}
        for destination in complete_address:
            #     distance = gmaps.distance_matrix(origin, destination, mode='driving', units='imperial',
            #                    avoid=None)["rows"][0]["elements"][0]["distance"]["value"] * 0.000621371 #meter to miles
            distance = random.random()
            if float(distance) <= walking_distance * 0.0018939:  # to do 需要改成正确的feet-mile系数
        ...
        result = [{'name': k[:len(k) - 21], 'distance': f'{round(v[0],3)} miles', 'rateRange': v[1][1], 'timeLimit': v[1][0]} for k, v in
